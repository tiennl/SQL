...
AS
	BEGIN
		IF NOT EXISTS (SELECT MaLT, SoHieuMB, MaPC FROM LICHTRINHMAYBAY WHERE MaLT = @MaLT AND SoHieuMB = @SoHieuMB AND MaPC = @MaPC)
			IF (@MaPC IS NULL OR EXISTS(
			SELECT MaPC FROM PHICONG WHERE MaPC=@MaPC))
			AND
			(@SoHieuMB IS NULL OR EXISTS(
			SELECT SoHieuMB FROM MAYBAY WHERE SoHieuMB=@SoHieuMB))
				INSERT INTO LICHTRINHMAYBAY VALUES (@MaLT, @SoHieuMB, @MaPC, @NoiDi, @NoiDen, @ThoiGianDi, @ThoiGianDen)
		ELSE PRINT 'Khong them duoc'
	END

SP_2 'LT003', 'VN01', 'PC02', 'TP HCM', 'Da Nang', '2016-05-20 14:00', '2016-05-20 16:00'
